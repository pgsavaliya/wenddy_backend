...
async function uploadReduceFile(file) {
    console.log("fillllllllllllllllllllllllllllllll", file)


    // return await  sharp(file, {raw})
    //   .toBuffer()
    //   .then(data => {
    ...
    for (const [index, siz] of size.entries()) {
        ...
        switch (siz) {
            ...
            case 2: {
                ...
                let awsImg = await uploadFile(await uploadReduceFile(image), filename, "reduceSize");
                console.log("awsImg ...........", awsImg);
