...
exports.cpMobileCheck = async (req, res, next) => {
  try {
    ...
    if (isMobile)
      response("The Mobile Number has already been used", {}, 404, res);
    else next();
  } catch (err) {
    response("something went wrong!!", err, 500, res);
  }
  ...
  try {
    if (req.body.type == "userLogin") {
      ...
      if (data) next();
        ...
        response(
          "number is not registered, please check the mobile number you entered",
          {},
          400,
          res
        );
    } else if (
      req.body.type == "contactUs" ||
      req.body.type == "userRegistration"
    ) {
      next();
    }
    ...
    // }
      ...
      response(
        "Only userLogin, contactUs and userRegistration are alloweded!!",
        {},
        400,
        res
      );
